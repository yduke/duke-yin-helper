...
require("import.save.tv.php");


// Find posts without a meta entry for the fruit custom field
$posts_without_movies = posts_without_meta( '_zmovies_json', 'tvshow_review', 'ids' );
...
<?php
$search = Movies::$TMDB->searchTVShow($post->post_title,'cl');
if( count($search) < 1 && preg_match("@:@", $post->post_title) ) {
    $parts = explode(":", $post->post_title, 2);
    if( count($parts) > 1 )
    {
        $title = trim($parts[1]);
        $search = Movies::$TMDB->searchTVShow($title,'cl');
    }
}
...
?>
                    ...
                    >
                        ...
                        <?php if($movie->getTvYear()) { ?>
                            (<?php echo $movie->getTvYear() ?>)
                        <?php } ?>
                    </option>
<?php } ?>
